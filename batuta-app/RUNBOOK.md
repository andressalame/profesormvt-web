...
- **CULQI (BYOK; tarjeta+Yape por API, para Peru con RUC) — ✅ ENCENDIDO 13-jul-2026:** el profe pega pk_live_/sk_live_ en Ajustes; la sk_ se cifra AES-GCM en reposo y NUNCA se expone al front. Cargo sincrono POST /v2/charges + webhook de respaldo que RE-VERIFICA el cargo con la sk_. Requiere RUC del profe.
  - CULQI_ENC_KEY ya cargado como secret en el worker (13-jul). Backup del valor: `~/.claude/scheduled-tasks/.secrets/batuta-culqi-enc-key.txt` (perm 600; movido del vault el 10-ago-2026 porque el vault viaja por Syncthing y R2; el archivo del vault quedó redactado). Si se pierde ese valor Y se borra el secret, las sk_ cifradas quedan ilegibles y los profes deben reconectar.
...
- **Secret:** ELEVENLABS_API_KEY en el worker; respaldo en `~/.claude/scheduled-tasks/.secrets/batuta-elevenlabs-api.key` (600; movido del vault el 10-ago-2026, el archivo del vault quedó redactado).
...
## Aterrizaje Cloud API del 3402 (asistente de Batuta) — CABLEADO 10-ago-2026

Preparacion del corte del +51 904 079 121 (linea Baileys 3402) a la Cloud API oficial. El corte NO se ejecuto: esto deja el aterrizaje listo. Plan completo en el vault: proyectos/batuta/Batuta - plan de corte 3402 a Cloud API (2026-08-10).md.

- **Ruta**: negocio `batuta-3402` en `wa_negocio` (rama negocio del webhook), KB de venta de Batuta portada de ~/Code/wa-asistentes/kb/batuta.md al JSON (voz del fundador con tuteo forzado, planes, FAQ, reglas). `kb.modelo = "sonnet"` = esta linea responde con Sonnet (los clientes siguen en Haiku; llamarClaudeOnboarding acepta el 5o parametro).
- **phone_id placeholder `51904079121`** (los digitos de la linea; un phone_number_id real nunca colisiona). El DIA DEL CORTE, tras registrar el numero en Meta, escribir el phone_number_id real con `curl -X POST https://batuta.lat/app/api/su/negocio-upsert -H "Authorization: Bearer $(cat .admin-token.local)" -H 'content-type: application/json' -d '{"id":"batuta-3402","nombre":"Batuta (asistente oficial)","phone_id":"<EL_REAL>"}'` — desde el 10-ago los campos omitidos (kb, enabled, cap_mes) se PRESERVAN. El payload completo de referencia vive en batuta-app/negocio-batuta-3402.json.
- **Aviso a Andres**: 1 correo por CHAT NUEVO (fila nueva en wa_negocio_lead), disparado por codigo en manejarNegocioWA via alertaCorreoAndres (Resend -> fallback AVISOS). No por mensaje.
- **Bandeja**: https://batuta.lat/app/su/bandeja (shell publico sin datos; pide el ADMIN_TOKEN una vez, queda en localStorage). Ver chats, responder a mano por Graph API y pausar la IA por chat (pausa = columna `pausa` en wa_negocio_lead; el mensaje entrante igual se guarda en el historial).
- **Endpoints su/ nuevos**: negocio-chats (lista) · negocio-chat?negocio=&telefono= (historial) · negocio-responder {negocio,telefono,texto} · negocio-pausa {negocio,telefono,pausa:on|off} · negocio-chat-borrar {negocio,telefono}.
- **E2E REAL corrido el 10-ago** con el numero de prueba de la WABA (phone_id 1149379364933769) apuntado temporalmente al negocio: webhook entrante simulado -> lead + correo de chat nuevo -> Sonnet respondio -> WhatsApp REAL entregado a 51989077928 -> pausa silencio la IA -> respuesta manual salio -> chat de prueba borrado y phone_id devuelto al placeholder. Token WHATSAPP_TOKEN del 22-jul: VIVO (su/wa-status ok).
- Historial de la rama negocio ahora guarda el turno del cliente SIEMPRE (aunque el envio falle o este en pausa): la bandeja nunca queda ciega.
